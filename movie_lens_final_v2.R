...
movielens %>% summarise(mov = length(unique(movieId)), users = length(unique(userId)), genres = length(unique(genres)))
...
## create function for accuracy measurement
RMSE <- function(true_ratings, predicted_ratings){ # set up error measurement for models
...
m_deltas %>% ggplot(aes(abs(delta), n)) + geom_point(alpha=0.25) + ggtitle("Absolute Residuals vs Number of Reviews") + 
  annotate("text", x = 2.5, y = 1000, label = "Movies with few reviews tend to\n have worse estimates") +
...
user_view %>%   
  ...
  annotate("text", x= 10, y=2000, label="Most users\n have left\n many\n reviews", color="blue")
